...
When enabled, Mobile constructs the browser-safe `@repo/conversation-sdk` client and sends a synthetic text turn to Chatterbox. `ConversationSessionService` owns request lifecycle and emits only pending, succeeded, failed, or aborted events. A new submission aborts the previous request; late or superseded responses cannot render. Safe API failures may be displayed, while raw provider/internal errors are replaced with generic PT-BR copy.
