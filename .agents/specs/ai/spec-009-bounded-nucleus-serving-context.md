...
# SPEC-009: Inject bounded Memory Nucleus context into Conversation serving
...
This spec is blocked by `SPEC-008`. Its branch remains contract-only until the first real conversation is accepted. The experiment tests the serving half of Memory Nucleus; it does not yet create memory from a completed PWA conversation.
