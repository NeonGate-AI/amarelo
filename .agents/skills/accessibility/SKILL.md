...
WCAG 2.2 Level AA is the normative target for Amarelo web surfaces. WCAG 2.5.8 defines a 24 by 24 CSS-pixel minimum with exceptions; Amarelo's product target is at least 44 by 44 CSS pixels for practical interactive targets whenever the control is not an inline-text or user-agent exception.

The repository currently defers automated verification. Do not run Lighthouse, axe automation, browser-automation audits, unit tests, integration tests, smoke tests, Playwright, or Cypress. Audit source statically, inspect rendered behavior manually when a human-accessible page is available, and label anything not directly observed at runtime as a hypothesis. A source audit cannot establish WCAG conformance.

## Evidence-led audit workflow

Use the following workflow under the current repository policy:

1. Identify the relevant page, state, viewport, theme, and interaction from the specification and source.
2. Inspect semantic HTML, names, roles, values, states, landmarks, heading structure, focus transitions, live regions, keyboard behavior, target geometry, contrast tokens, reflow constraints, orientation, and reduced-motion handling in source.
3. When a rendered page is already available for human inspection, exercise the affected flow manually with keyboard and a screen reader. Do not start browser automation or Lighthouse.
4. Separate observed source facts, manually observed runtime facts, and unverified runtime hypotheses.
5. Fix the source, then repeat the same static review and permitted manual interaction.

Automated tools detect only a subset of accessibility barriers. When automation is re-enabled by a future owner decision, its score still must not be treated as WCAG conformance.
...
| **AA** | Standard compliance | Normative Amarelo target |
...
Amarelo uses **44 × 44 CSS pixels** as its practical product target whenever the control is not covered by an inline-text or user-agent exception.
...
Deferred by repository policy. Do not install or run Lighthouse, axe CLI, browser automation, or an automated accessibility suite in the current phase. Record those checks as deferred rather than silently substituting a score or claiming conformance.
...
- [ ] **Target size:** Interactive elements meet the 24×24px WCAG minimum where applicable and Amarelo's 44×44px practical target
