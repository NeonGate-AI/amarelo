...
The repository currently permits static source review and manual inspection only. Do not run Lighthouse, PageSpeed automation, browser traces, synthetic performance suites, browser automation, or automated tests. Use existing owner-provided field data only when it is already available and its scope is known. Without runtime evidence, report potential LCP, INP, and CLS causes as hypotheses; do not report measured values, pass/fail results, or claimed improvements.

## Measure before optimizing

Under the current repository policy:

1. Define the route, state, viewport, and expected primary content from the harness and source.
2. Inspect likely LCP candidates, asset discovery, font loading, main-thread work, event handlers, layout reservations, dynamic insertion, and animation in source.
3. Inspect an already available rendered page manually when useful, without starting an automated audit or trace.
4. Label every metric-related conclusion as an observed source fact, manually observed runtime fact, existing field datum, or hypothesis.
5. After a change, repeat the same static review and permitted manual inspection. Record quantitative lab and field validation as deferred.

Read [the performance measurement workflow](../performance/references/MEASUREMENT.md) only as future measurement guidance. Do not execute its automated steps until the owner reverses the current policy.
...
## Evidence sources in the current phase

| Source | Use |
|--------|-----|
| Source inspection | Identify code and resource patterns that may affect LCP, INP, or CLS without claiming a measured failure |
| Manual rendered inspection | Observe qualitative loading, interaction, and layout behavior without inventing metric values |
| Existing owner-provided CrUX, Search Console, or RUM evidence | Prioritize known user impact when the route, percentile, period, and device scope are explicit |

Browser traces, Lighthouse, PageSpeed automation, raw observer instrumentation, and new RUM collection are deferred. When a future owner decision re-enables measurement, read [the measurement workflow](../performance/references/MEASUREMENT.md) and [the first-party RUM reference](../performance/references/RUM.md), and never compare a single lab value directly with a field p75 as if they were equivalent samples.
