...
export class MemoryShadowAgent extends ConversationAgentPort {
  ...
  constructor(
    ...
    private readonly executor: MemoryShadowExecutor,
    private readonly request: MemoryShadowRequest
  ) {
    super()
    this.id = agent.id
  }
  async invoke(
    ...
    try {
      this.executor.schedule({
        ...this.request,
        invocation,
        controlUsage: result.usage
      })
    } catch {
      /* Shadow cannot change the delivered result. */
    }
    return result
  }
}
