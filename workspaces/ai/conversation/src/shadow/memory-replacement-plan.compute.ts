...
export function createMemoryReplacementPlan(
  ...
  if (
    ...
    !Number.isSafeInteger(recentBufferTokens) ||
    recentBufferTokens < 1 ||
    recentBufferTokens > 600
  ) {
  ...
  if (current === undefined || current.role !== 'user')
    throw new Error('Current user turn is missing')
  const history = messages.slice(0, -1)
  const memory = MemorySearchResultSchema.parse(rawMemory)
  const projection = Object.freeze(
    memory.items.map(createMemorySearchContextProjection)
  )
  const projectionTokens = projection.reduce(
    (sum, item) => sum + estimateMemorySearchContextTokens(item),
    0
  )
  if (
    projectionTokens > 600 ||
    projectionTokens > memory.tokenBudget.effectiveTokens
  ) {
  ...
  const remaining =
    invocation.routing.budget.contextTokens - currentTokens - projectionTokens
  if (remaining < 0)
    throw new Error('Memory replacement exceeds the actual context budget')
  ...
  const treatment = Object.freeze({
    ...control,
    memory: projection,
    messages: Object.freeze([...recent.messages, current])
  })
  // An unchanged complete history plus Memory would be the prohibited additive treatment.
  if (
    projection.length > 0 &&
    history.length > 0 &&
    recent.messages.length === history.length
  ) {
    throw new Error(
      'Memory projection cannot be appended to the complete control history'
    )
  }
  return Object.freeze({
    ...
    controlHash: createHash('sha256')
      ...
      .digest('hex'),
    treatmentHash: createHash('sha256')
      .update(JSON.stringify(treatment))
      .digest('hex'),
    controlComparableTokensEstimated: history.reduce(
      (sum, message) =>
        sum + estimateConversationMessageTokens(message.content),
      0
    ),
    treatmentComparableTokensEstimated:
      recent.estimatedTokens - currentTokens + projectionTokens
  })
}
