...
export class MemoryExperimentAgent extends ConversationAgentPort {
  ...
  constructor(
    private readonly agent: ConversationAgentPort,
    private readonly controller: MemoryExperimentController,
    private readonly request: MemoryExperimentRequest
  ) {
    super()
    this.id = agent.id
  }
  ...
  async invoke(
    ...
    try {
      ...
      try {
        ...
        if (/^[A-Za-z0-9][A-Za-z0-9._:-]{0,199}$/.test(this.request.reportId))
          this.controller.record({
            schemaVersion: 'memory-experiment-turn-v1',
            specId: 'SPEC-017',
            reportId: this.request.reportId,
            experimentId: selection.policy?.experimentId ?? null,
            policyVersion: selection.policy?.version ?? null,
            versions,
            assignment: selection.assignment,
            subjectHash: selection.subjectHash,
            decision: selection.decision,
            controlComparableTokensEstimated:
              selection.plan?.controlComparableTokensEstimated ??
              invocation.messages
                .slice(0, -1)
                .reduce(
                  (sum, message) =>
            ...
            treatmentComparableTokensEstimated:
              selection.plan?.treatmentComparableTokensEstimated ?? null,
            totalModelInputTokens: result?.usage?.inputTokens ?? null,
            invocationLatencyMs: performance.now() - started,
            agentInvocations: 1,
            modelCalls: null,
            costBrl: null,
            voiceEvidence: 'not-measured',
            outcome: result === undefined ? 'failed' : 'returned'
          })
      } catch {
        /* Reports contain no response or raw provider error. */
      }
    }
  }
}
