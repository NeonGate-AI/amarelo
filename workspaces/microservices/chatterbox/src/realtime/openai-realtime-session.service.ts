...
import type {
  RealtimeMemoryDependencies,
  RealtimeSessionStatus,
  RealtimeMemoryStatus
} from './realtime-memory.contract'
...
export class OpenAiRealtimeSessionService {
  ...
  async start(
    ...
    if (
      ...
      this.#sessions.size >= (this.#options.maxSessions ?? 2)
    )
      throw new Error('Realtime session unavailable')
    if (
      [...this.#sessions.values()].some(
        (session) =>
          session.context.subjectId === context.subjectId &&
          session.context.tenantId === context.tenantId
      )
    )
    ...
    session.timer = setInterval(() => {
      if (
        ...
        Date.now() - session.lastHeartbeat > 35_000
      )
        void this.#stop(session)
    }, 5_000)
    session.timer.unref()
    try {
      const call = await createOpenAiRealtimeSession({
        apiKey: this.#options.apiKey,
        sdp,
        model: this.#options.model,
        voice: this.#options.voice,
        transcriptionModel: this.#options.transcriptionModel,
        memoryEnabled: this.#options.memory !== undefined,
      ...
      if (session.closed || Date.now() >= context.expiresAtMs)
  ...
  status(context: AuthenticatedConversationContext): RealtimeSessionStatus {
    ...
    if (session === undefined)
      return {
        state: 'stopped',
        memory: { status: 'idle', acceptedCount: null },
        expiresAtMs: context.expiresAtMs
      }
    session.lastHeartbeat = Date.now()
    return {
      state: session.ready ? 'active' : 'unavailable',
      memory: { status: session.memory, acceptedCount: session.acceptedCount },
      expiresAtMs: session.context.expiresAtMs
    }
  }
  ...
  #owned(context: AuthenticatedConversationContext): Session | undefined {
    ...
    return session !== undefined &&
      session.context.tenantId === context.tenantId &&
      session.context.subjectId === context.subjectId &&
      session.context.actorId === context.actorId &&
      session.context.authenticationSessionId ===
        context.authenticationSessionId
      ? session
      : undefined
  }
  ...
  async #connect(session: Session): Promise<void> {
    const socket = new WebSocket(
      `wss://api.openai.com/v1/realtime?call_id=${encodeURIComponent(session.callId!)}`,
      {
        ...
        handshakeTimeout: Math.min(this.#options.timeoutMs, 10_000),
        maxPayload: 256 * 1024
      }
    )
    ...
    socket.on('message', (raw) => {
      ...
      if (event.type === 'response.created') {
        ...
        if (typeof id === 'string') {
          ...
          if (session.responseTurns.size > 128)
            session.responseTurns.delete(
              session.responseTurns.keys().next().value!
            )
        }
      }
      if (
        ...
        return
      if (event.type === 'error' || session.pending >= 32) {
        void this.#stop(session)
        return
      }
      ...
      session.chain = session.chain
        .then(() => this.#event(session, event, generation))
        .catch(() => {
    ...
    await new Promise<void>((resolve, reject) => {
      const timer = setTimeout(
        () => reject(new Error('Realtime sideband unavailable')),
  ...
  async #event(
    ...
    if (event.type === 'response.done') {
      ...
      if (
        ...
        !safeId(response.id) ||
        this.#seen(session, `usage:${response.id}`) ||
        memory === undefined
      )
        return
      ...
      if (ledger !== null)
        await ledger.append(
          mapOpenAiRealtimeUsage({
            scope: memory.createScope(context),
    ...
    if (
      ...
      if (
        ...
        !safeId(event.item_id) ||
        typeof event.transcript !== 'string' ||
        event.transcript.length > 4_000 ||
        event.transcript.trim().length === 0
      )
        return
      const id = `realtime:${createHash('sha256')
        .update(
          JSON.stringify([
            session.callId,
            event.item_id,
    ...
    if (
      ...
      event.name !== 'memory_search' ||
      typeof event.call_id !== 'string' ||
      !safeId(event.call_id) ||
      this.#seen(session, `tool:${event.call_id}`)
    )
      return
    const responseGeneration =
      typeof event.response_id === 'string'
        ? (session.responseTurns.get(event.response_id) ?? generation)
        : generation
    ...
    try {
      const argumentsValue =
        typeof event.arguments === 'string' && event.arguments.length <= 8_192
          ? object(JSON.parse(event.arguments))
          : {}
      if (
        memory !== undefined &&
        typeof argumentsValue.query === 'string' &&
        argumentsValue.query.trim().length > 0 &&
        argumentsValue.query.length <= 4_000
      ) {
        const client = memory.createMemoryClient(context)
        const search =
          this.#options.recall === undefined
            ? await client.search({
        ...
        const allowed =
          consent.entries.some(
            (entry) =>
              entry.purpose === 'conversation.support' &&
              entry.status === 'granted'
          ) && search.governance.consentVersion === consent.version
        if (allowed) {
          result = {
            status: 'available',
            trust: 'untrusted-memory-data',
            items: search.items.map((item) => item.context),
    ...
    if (
      session.closed ||
      session.generation !== responseGeneration ||
      Date.now() >= context.expiresAtMs
    )
      return
    this.#send(session, {
  ...
  #seen(session: Session, key: string): boolean {
    ...
    if (session.handled.size > 512)
      session.handled.delete(session.handled.values().next().value!)
    return false
  }
  ...
  async #stop(session: Session): Promise<void> {
    ...
    if (this.#sessions.get(session.context.conversationId) === session)
      this.#sessions.delete(session.context.conversationId)
