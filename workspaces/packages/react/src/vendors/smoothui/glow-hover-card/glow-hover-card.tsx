...
export function GlowHover(props: GlowHoverProps) {
  const { className = '', glowIntensity = 0.15, items, maskSize = 400 } = props
  ...
  function applyGlowStyles(
    ...
    const typedElement = element as ReactElement<GlowElementProps>
    const existingStyle = typedElement.props.style ?? {}
    const existingClassName = typedElement.props.className ?? ''
    ...
    >
      ...
      <div className="contents">
        {items.map((item, index) => {
          const element = item.element as ReactElement<GlowElementProps>
          const existingRef = element.props.ref

          return cloneElement(element, {
            key: item.id,
            ref: (elementNode: HTMLElement | null) => {
              itemRefs.current[index] = elementNode
              if (typeof existingRef === 'function') {
                existingRef(elementNode)
              } else if (existingRef && typeof existingRef === 'object') {
                existingRef.current = elementNode
              }
            }
          })
        })}
      ...
      {!shouldReduceMotion && (
        ...
        >
          {items.map((item, index) => {
            const glowElement = applyGlowStyles(
              item.element,
              item.theme,
              true
            ) as ReactElement<GlowElementProps>

            return cloneElement(glowElement, {
              key: item.id,
              ref: (elementNode: HTMLElement | null) => {
                overlayItemRefs.current[index] = elementNode
              }
            })
          })}
        </div>
...
export function GlowHoverCards(props: GlowHoverCardsProps) {
  const { className, glowIntensity, items, maskSize } = props
