...
void main(){
  ...
  vec3 brandYellow = vec3(0.98, 0.84, 0.08);
  vec3 warmLight = vec3(1.0, 0.90, 0.33);
  vec3 seafoam = vec3(0.45, 0.77, 0.59);
  vec3 porcelain = vec3(0.98, 0.98, 0.96);
  vec3 color = mix(brandYellow, warmLight, smoothstep(-0.18, 0.18, sin((uv.y + n) * PI * 2.0)));
...
export function Chroma(props: ChromaProps) {
  const {
    children,
    className,
  ...
  const isKeyChanging = previousKey.current !== transitionKey
  const visibleChildren =
    isActive || isKeyChanging ? renderedChildren : children
  ...
  return (
    ...
    >
      <div className="relative z-0">{visibleChildren}</div>
      <canvas
        className={cn(
