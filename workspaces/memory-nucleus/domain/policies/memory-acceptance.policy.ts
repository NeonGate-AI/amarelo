...
export class MemoryAcceptancePolicy {
  ...
  assertAcceptable(candidate: MemoryAcceptanceCandidate, judgment: MemoryJudgment): void {
    if (!judgment.shouldPersist) {
      throw new Error(`judgment ${judgment.decision} cannot activate canonical memory`)
    }
    if (candidate.confidence < this.minimumConfidence) {
      throw new Error('candidate confidence is below the canonical-memory threshold')
    }
