...
const evalRetrievalRecordsPayloadFreeTrace: MemoryRetrievalEvalCase = async () => {
  const repository = new InMemoryScopedMemoryRepository([traceRecord, traceRecord])
  ...
  const result = await retrieveAuthorizedMemory(
    ...
    }),
    {
      ...
      observer: { record(trace) { traces.push(trace) } }
    }
  )
  ...
  assert.equal(trace.tokenEstimatorVersion, MEMORY_RETRIEVAL_TOKEN_ESTIMATOR_VERSION)
  assert.deepEqual(trace.selectedMemoryIds, result.items.map(({ id }) => id))
  assert.deepEqual(trace.candidateDecisions.map(({ decision }) => decision).sort(), ['duplicate', 'selected'])
...
const evalObserverFailureDoesNotBreakServing: MemoryRetrievalEvalCase = async () => {
  ...
  const result = await retrieveAuthorizedMemory(
    ...
    {
      ...dependencies,
      observer: { async record() { throw new Error('synthetic sink unavailable') } }
    }
  )
  assert.equal(result.items.length, 1)
  return { name: 'observability failure never becomes a serving authority failure' }
}

const evalObserverDeadlineDoesNotBlockServing: MemoryRetrievalEvalCase = async () => {
  const repository = new InMemoryScopedMemoryRepository([traceRecord])
  const dependencies = memoryRetrievalDependencies(repository)
  const result = await retrieveAuthorizedMemory(
    authorizedMemoryQuery({
  ...
  assert.equal(result.items.length, 1)
  return { name: 'observability deadline bounds telemetry without blocking retrieval' }
}
