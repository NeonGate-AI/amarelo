...
export interface MemoryIntegrityFixtureStore {
  ...
  ): Promise<{ operation: MemoryIntegrityStoreOperation; matches: boolean }>
...
export class Neo4jMemoryIntegrityFixtureStore
  implements MemoryIntegrityFixtureStore
{
  readonly nonDefault: boolean
  ...
  constructor(
    ...
    if (!options.database || options.disposableSyntheticStore !== true)
      throw new Error('Disposable integrity store configuration required')
  ...
  async poison(
    ...
    const result = await this.query(
      `MATCH (m:Memory {memoryId: $memoryId, tenantId: $tenantId, subjectId: $subjectId})
       ...
       RETURN m.memoryId AS id`,
      scope,
      {
        memoryId,
        sourceKind:
          kind === 'assistant' ? 'assistant-text' : 'synthetic-transcript',
        actorId: kind === 'assistant' ? scope.actorId : randomUUID()
      }
    )
    if (result.records.length !== 1)
      throw new Error('Synthetic poison target missing in configured store')
  }
  ...
  async conflict(
    ...
    const result = await this.query(
      `MATCH (a:Memory {memoryId: $leftId, tenantId: $tenantId, subjectId: $subjectId})-[:HAS_VERSION]->(av:MemoryVersion)
       ...
       RETURN av.semanticKey AS semanticKey, bv.recordJson AS recordJson`,
      scope,
      { leftId, rightId }
    )
    ...
    if (
      !row ||
      typeof row.get('recordJson') !== 'string' ||
      typeof row.get('semanticKey') !== 'string'
    )
      throw new Error('Synthetic conflict target missing')
    const record: unknown = JSON.parse(row.get('recordJson'))
    if (!record || typeof record !== 'object' || Array.isArray(record))
      throw new Error('Synthetic conflict record invalid')
    await this.query(
      `MATCH (m:Memory {memoryId: $rightId, tenantId: $tenantId, subjectId: $subjectId})-[:HAS_VERSION]->(v:MemoryVersion)
       WHERE m.currentVersionId = v.versionId SET v.semanticKey = $key, v.recordJson = $json`,
      scope,
      {
        rightId,
        key: row.get('semanticKey'),
        json: JSON.stringify({ ...record, semanticKey: row.get('semanticKey') })
      }
    )
  }
  ...
  async observe(
    ...
    const condition =
      operation === 'supersede'
        ? 'm.version >= 2'
        : ['suppress', 'replay', 'restore', 'reindex', 'rebuild'].includes(
              operation
            )
          ? 'EXISTS { MATCH (:MemorySuppression {canonicalIdentityKey: m.canonicalIdentityKey}) }'
          : "m.state = 'active'"
    const result = await this.query(
    ...
    return {
      ...
      matches:
        ...
        result.records[0]?.get('valid') === true &&
  ...
  async restoreHead(
    ...
    await this.query(
      `MATCH (m:Memory {memoryId: $memoryId, tenantId: $tenantId, subjectId: $subjectId})-[:HAS_VERSION]->(v:MemoryVersion)
       ...
       SET m.state = 'active', m.currentVersionId = v.versionId, m.version = v.version`,
      scope,
      { memoryId }
    )
  }
  ...
  async reindex(): Promise<void> {
    const result = await this.query(
      `SHOW INDEXES YIELD name, type, labelsOrTypes WHERE type = 'FULLTEXT' AND 'MemoryVersion' IN labelsOrTypes RETURN name`,
      null
    )
    ...
    if (
      ...
      throw new Error('Synthetic full-text index missing')
  ...
  async rebuild(scope: MemoryRequestScope): Promise<void> {
    ...
    const rows = result.records.map((row) => {
      ...
      const key =
        'semanticKey' in record && typeof record.semanticKey === 'string'
          ? record.semanticKey
          : ''
      return {
        versionId,
        searchableText: `${key} ${record.statement}`
          .normalize('NFKD')
          .replace(/\p{M}/gu, '')
    ...
    await this.query(
      `UNWIND $rows AS row
       ...
       SET v.searchableText = row.searchableText`,
      scope,
      { rows }
    )
  }
  ...
  async cleanup(scope: MemoryRequestScope): Promise<void> {
    ...
    await this.query(
      'MATCH (n) WHERE n.tenantId = $tenantId AND n.subjectId = $subjectId DETACH DELETE n',
      scope
    )
