...
export const MEMORY_INTEGRITY_STORE_OPERATIONS = [
  'write', 'retrieve', 'supersede', 'suppress', 'replay', 'restore', 'reindex', 'rebuild'
] as const

export const MEMORY_INTEGRITY_SOURCE_CASES = [
  'assistant-only', 'assistant-acknowledgment', 'assistant-repetition', 'forged-role', 'delegate', 'inactivity'
] as const
...
export const MemoryIntegrityReportSchema = z.object({
  ...
  sourceChecks: z.array(z.object({
    caseId: z.string().min(1).max(100),
    outcome: z.enum(['rejected', 'skipped', 'unsupported-accepted', 'unavailable'])
  }).strict()),
  store: z.object({
...
export function serializeMemoryIntegrityReport(report: MemoryIntegrityReport): string {
...
export type MemoryIntegrityStoreOperation = typeof MEMORY_INTEGRITY_STORE_OPERATIONS[number]
