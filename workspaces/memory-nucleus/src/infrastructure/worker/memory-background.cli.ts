...
async function main() {
  if (process.env.MEMORY_BACKGROUND_ENABLED !== 'true')
    throw new Error('Set MEMORY_BACKGROUND_ENABLED=true to start the worker explicitly')
  const modelId = required('MEMORY_EXTRACTION_MODEL')
  const extractor = new LangChainMemoryExtractor(
    new ChatOpenAI({ apiKey: required('OPENAI_API_KEY'), model: modelId, maxRetries: 0, timeout: 20_000, temperature: 0 }),
    { modelId, providerId: 'openai', version: 'memory-background-provider-v1', deadlineMilliseconds: 20_000 }
  )
  const runtime = await createNeo4jMemoryBackgroundRuntime({
    uri: required('MEMORY_NEO4J_URI'), username: required('MEMORY_NEO4J_USERNAME'),
    password: required('MEMORY_NEO4J_PASSWORD'), database: required('MEMORY_NEO4J_DATABASE'),
    extractor
  })
  let worker
  try {
    worker = await startMemoryBackgroundWorker({
      runtime, redisQueueUrl: required('MEMORY_REDIS_QUEUE_URL'),
      redisCacheUrl: required('MEMORY_REDIS_CACHE_URL'),
      concurrency: Number(process.env.MEMORY_WORKER_CONCURRENCY ?? '1'),
      observe: (report) => { process.stdout.write(JSON.stringify(report) + '\n') }
    })
  } catch (error) { await runtime.close(); throw error }
  const metrics = setInterval(() => {
    void worker.metrics().then((report) => process.stdout.write(JSON.stringify(report) + '\n')).catch(() => undefined)
  }, 30_000)
  let closing = false
  const close = async () => {
    if (closing) return
...
void main().catch(() => {
  ...
  process.stderr.write('Memory background worker could not start or close; inspect server configuration.\n')
  process.exitCode = 1
})
