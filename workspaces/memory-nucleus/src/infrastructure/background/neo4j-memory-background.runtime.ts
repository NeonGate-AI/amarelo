...
} from '@infrastructure/database/neo4j'
...
const constraints = [
  [
    'memory_background_batch_identity_v1',
    'MemoryBackgroundBatch',
    'identityKey'
  ],
  ...
  [
    'memory_background_candidate_id_v1',
    'MemoryBackgroundCandidate',
    'candidateId'
  ],
  [
    'memory_background_delta_identity_v1',
    'MemoryBackgroundDelta',
    'identityKey'
  ]
] as const
const result = (
  status: MemoryBackgroundProcessResult['status'],
...
class Neo4jBackgroundStore implements MemoryBackgroundStore {
  constructor(
    ...
    private readonly options: Neo4jMemoryBackgroundOptions,
  ...
  private async transaction<T>(
    ...
    try {
      return await session.executeWrite(work, { timeout: 15_000 })
    } finally {
      await session.close()
    }
  }
  ...
  private async locked<T>(
    ...
    const assertRequest = () => {
      try {
        ...
        throw new BackgroundAuthorityError(
          'Background Memory request has expired or is invalid'
        )
      }
    }
    assertRequest()
    return this.transaction(async (transaction) => {
      if (!(await isNeo4jMemoryTransactionSchemaReady(transaction)))
        throw new Error('Memory schema is unavailable')
      const head = await transaction.run(
        ...
        {
          scopeKey: neo4jMemoryScopeKey(scope),
      ...
      if (
        ...
        head.records[0]?.get('status') !== 'granted' ||
        (expected !== undefined && version !== expected)
      )
        throw new BackgroundAuthorityError(
          'Background Memory authority is unavailable'
        )
  ...
  async ingest(
    scope: MemoryRequestScope,
    trusted: TrustedMemorySource,
    formationSignal: MemoryFormationSignal,
    profile: MemoryBackgroundProfile
  ): Promise<MemoryBackgroundIngestResult> {
    assertNeo4jMemoryScope(scope, this.now())
    z.enum(['free', 'paid', 'internal']).parse(profile)
    z.enum(['none', 'eligible-source-delta', 'explicit-memory-request']).parse(
      formationSignal
    )
    if (profile === 'free' || formationSignal === 'none')
      return {
        status: 'skipped',
        reason:
          profile === 'free'
            ? 'free-background-disabled'
            : 'no-formation-signal'
      }
    const submittedSource = prepareEligibleMemorySource(
      trusted,
      scope,
      this.now()
    )
    if (submittedSource === null)
      return { status: 'skipped', reason: 'no-subject-evidence' }
    const words = submittedSource.text
    ...
    return this.locked<MemoryBackgroundIngestResult>(
      scope,
      undefined,
      async (transaction, consentVersion) => {
        const buffered = await this.bufferSource(
          transaction,
          scope,
          submittedSource,
          profile,
        ...
        const eventId = neo4jMemoryFingerprint([
          'memory-background-enqueued-v1',
          batchId
        ])
        const job = MemoryBackgroundJobSchema.parse({
          schemaVersion: 'memory-background-job-v1',
          eventId,
          batchId,
          tenantId: scope.tenantId,
          subjectId: scope.subjectId,
        ...
        const prepared = prepareMemoryCuration(
          request,
          DEFAULT_MEMORY_CURATION_POLICY,
          normalizer
        )
        if (!prepared.decision.eligible)
          return { status: 'buffered', reason: 'below-minimum-content' }
        const identityKey = neo4jMemoryFingerprint([
          'memory-background-batch-v1',
          scopeKey,
          turns.map((turn) => turn.identityKey)
        ])
        const fingerprint = neo4jMemoryFingerprint([
          'memory-eligible-evidence-v1',
          source.text,
          turns.map((turn) => [turn.identityKey, turn.sourceFingerprint])
        ])
        const previous = await transaction.run(
          'MATCH (b:MemoryBackgroundBatch {identityKey: $identityKey}) RETURN b.fingerprint AS fingerprint, b.jobJson AS jobJson',
          { identityKey }
        )
        if (previous.records.length > 0) {
          if (previous.records[0]?.get('fingerprint') !== fingerprint)
            throw new Error('Background source identity collision')
          return {
            status: 'duplicate',
            job: MemoryBackgroundJobSchema.parse(
              JSON.parse(previous.records[0]?.get('jobJson'))
            )
          }
        }
        ...
        if (registered.records.length !== turns.length)
          throw new Error('Background source version has conflicting evidence')
        const evidenceId = randomUUID()
        const at = this.now().toISOString()
        const observedAt = source.turns.reduce(
          (latest, turn) =>
            Date.parse(turn.observedAt) > Date.parse(latest)
              ? turn.observedAt
              : latest,
          source.turns[0]?.observedAt ?? at
        )
         ...
         CREATE (e)-[:HAS_SOURCE_TURN]->(s)`,
          {
            ...
            text: source.text,
            turns,
            turnsJson: JSON.stringify(request.turns),
            sourceTurnIds: turns.map((turn) => turn.identityKey),
            sourceTurnVersions: turns.map((turn) => turn.sourceTurnVersion),
  ...
  private async bufferSource(
    ...
    | { source: EligibleMemorySource; keys: string[] }
  ...
  > {
    ...
    for (const turn of source.turns) {
      ...
      if (
        registered.records[0]?.get('scopeKey') !== scopeKey ||
        registered.records[0]?.get('fingerprint') !== fingerprint
      )
        throw new Error(
          'Buffered source conflicts with its immutable turn version'
        )
      const newer = await transaction.run(
         ...
         WHERE d.sourceTurnVersion>$version RETURN d.identityKey AS identityKey LIMIT 1`,
        {
          ...
          sourceTurnId: turn.sourceTurnId,
          version: turn.sourceTurnVersion
        }
      )
      if (newer.records.length > 0)
        throw new Error('Buffered source version is stale')
      await transaction.run(
      ...
      if (
        delta?.get('fingerprint') !== fingerprint ||
        delta.get('consentVersion') !== consentVersion
      )
        throw new Error('Buffered source version identity mismatch')
      if (delta.get('status') === 'consumed')
        duplicateJob = MemoryBackgroundJobSchema.parse(
          JSON.parse(delta.get('jobJson'))
        )
      ...
      await transaction.run(
           ...
           o.schemaVersion='memory-outbox-v1',o.status='pending',o.createdAt=$at,o.attempts=0`,
        {
          ...
          scopeKey,
          identityKey,
          eventId: neo4jMemoryFingerprint([
            'memory-background-buffered-v1',
            identityKey
          ]),
          at: this.now().toISOString()
        }
    ...
    if (!pendingSubmission && duplicateJob !== undefined)
      return { job: duplicateJob }
    ...
    for (const row of pending.records) {
      const turn = JSON.parse(
        row.get('turnJson')
      ) as EligibleMemorySource['turns'][number]
      if (
        selected.length >= 20 ||
        characters + turn.text.length + (selected.length > 0 ? 1 : 0) > 4_000
      )
        continue
    ...
    const combined = prepareEligibleMemorySource(
      { events: selected },
      scope,
      this.now()
    )
  ...
  private request(
    scope: MemoryRequestScope,
    turns: readonly { id: string; observedAt: string; text: string }[],
    formationSignal: MemoryFormationSignal,
    decisionId: string
  ) {
    return MemoryCurationRequestSchema.parse({
  ...
  async pending(limit: number): Promise<readonly MemoryBackgroundJob[]> {
    ...
    const rows = await this.driver.executeQuery(
      `MATCH (o:OutboxEvent {eventType:'memory.background-enqueued.v1',status:'pending'})
       RETURN o.jobJson AS jobJson ORDER BY o.createdAt LIMIT $limit`,
      { limit: neo4j.int(limit) },
      { database: this.options.database }
    )
    return rows.records.map((row) =>
      MemoryBackgroundJobSchema.parse(JSON.parse(row.get('jobJson')))
    )
  }
  ...
  async markPublished(eventId: string): Promise<void> {
    ...
    await this.driver.executeQuery(
      ...
      { eventId, at: this.now().toISOString() },
      { database: this.options.database }
    )
  }
  ...
  private async assertFence(
    ...
    if (found.records.length !== 1)
      throw new Error('Background worker claim is unavailable')
  }
  ...
  async open(job: MemoryBackgroundJob, attempt: number) {
    const metadata = await this.driver.executeQuery(
      ...
      { ...job, jobJson: JSON.stringify(job) },
      { database: this.options.database }
    )
    ...
    if (['skipped', 'quarantined'].includes(row.get('status')))
      return result('skipped', 'terminal-batch')
    const scope = JSON.parse(row.get('scopeJson')) as MemoryRequestScope
    const consentVersion = z
      .number()
      .int()
      .positive()
      .parse(row.get('consentVersion'))
    let opened:
      | {
          binding: ExecutionBinding
          request: ReturnType<Neo4jBackgroundStore['request']>
        }
      | MemoryBackgroundProcessResult
    try {
      opened = await this.locked(scope, consentVersion, async (transaction) => {
        const claim = await transaction.run(
             ...
             b.attempts AS attempts,e.turnsJson AS turnsJson`,
          {
            ...
            claimId: randomUUID(),
            leaseUntil: new Date(
              Math.min(scope.expiresAtMs, this.now().getTime() + this.leaseMs)
            ).toISOString()
          }
        )
        ...
        if (
          ...
          claimed.get('attempts') > this.maxAttempts
        ) {
          await transaction.run(
            "MATCH (b:MemoryBackgroundBatch {batchId:$batchId}) SET b.status='quarantined',b.reason='attempts-exhausted'",
            job
          )
          return result('quarantined', 'attempts-exhausted')
        }
        const current = await transaction.run(
          'MATCH (b:MemoryBackgroundBatch {batchId:$batchId}) RETURN b.claimId AS claimId,b.leaseUntil AS leaseUntil',
          job
        )
        const binding: ExecutionBinding = {
          ...
          scope,
          consentVersion,
          claimId: current.records[0]?.get('claimId'),
          leaseUntil: current.records[0]?.get('leaseUntil'),
          fence: claimed.get('fence'),
          evidenceId: claimed.get('evidenceId'),
        ...
        return {
          ...
          request: this.request(
            scope,
            JSON.parse(claimed.get('turnsJson')),
            claimed.get('formationSignal'),
            binding.claimId
          )
        }
      })
    ...
    const execution: MemoryBackgroundExecution = {
            ...
            return await this.locked(
              ...
              async (transaction) => {
                ...
                return {
                  ...
                  expiresAt: binding.leaseUntil
                }
              }
            )
          } catch (error) {
            if (error instanceof BackgroundAuthorityError) return null
            throw error
          }
        }
      },
      ...
      beforeModel: async () => {
        await this.locked(scope, consentVersion, (transaction) =>
          this.assertFence(transaction, binding)
        )
      ...
      complete: async (curated) => {
        ...
        return {
          status: curated.status === 'persisted' ? 'completed' : curated.status,
          reason: curated.reason,
          modelCalls: curated.usage.modelCalls,
          candidateCount: curated.usage.candidateCount,
          accepted
        }
      },
  ...
  private async claimSource(
    ...
    return this.locked(
      ...
      async (transaction) => {
        ...
        if (previous?.get('binding') !== fingerprint)
          throw new Error('Curation run identity collision')
        if (previous.get('status') === 'completed')
          return {
            status: 'duplicate' as const,
            runId: previous.get('runId') as string,
            claimId: null,
            claimExpiresAt: null
          }
        if (
          previous.get('claimId') &&
          previous.get('claimId') !== binding.claimId &&
          Date.parse(previous.get('leaseUntil')) > this.now().getTime()
        )
          return {
            status: 'in-progress' as const,
            runId: null,
            claimId: null,
            claimExpiresAt: previous.get('leaseUntil') as string
          }
        await transaction.run(
          'MATCH (r:MemoryBackgroundRun {idempotencyKey:$idempotencyKey}) SET r.claimId=$claimId,r.leaseUntil=$leaseUntil,r.batchId=$batchId',
          {
            idempotencyKey: input.idempotencyKey,
            claimId: binding.claimId,
            leaseUntil: binding.leaseUntil,
            batchId: binding.job.batchId
          }
        )
        return {
          status: 'claimed' as const,
          claimId: binding.claimId,
          claimExpiresAt: binding.leaseUntil,
          runId: null
        }
      }
    )
  ...
  private async saveRun(
    ...
    return this.locked(
      ...
      async (transaction) => {
        await this.assertFence(transaction, binding)
        if (
          input.claimId !== binding.claimId ||
          input.authorizationDecisionId !== binding.claimId ||
          input.tenantId !== binding.scope.tenantId ||
          input.subjectId !== binding.scope.subjectId ||
          input.actorId !== binding.scope.actorId
        )
        ...
        const activated = await activateBackgroundCandidates(transaction, {
          scope: binding.scope,
          batchId: binding.job.batchId,
        ...
        await transaction.run(
         ...
         CREATE (:OutboxEvent {eventId:$eventId,batchId:$batchId,tenantId:$tenantId,subjectId:$subjectId,requestId:$requestId,
           eventType:'memory.background-completed.v1',schemaVersion:'memory-outbox-v1',status:'pending',createdAt:$at,attempts:0})`,
          {
            ...binding.job,
            idempotencyKey: input.idempotencyKey,
            candidateIds: activated.candidateIds,
            accepted: activated.accepted,
            candidateCount: input.candidates.length,
            at: this.now().toISOString(),
            eventId: neo4jMemoryFingerprint([
              'memory-background-completed-v1',
              binding.job.batchId
            ])
          }
        )
        return {
          ...
          result: {
            ...
            runId: run.records[0]?.get('runId') as string,
            candidateIds: activated.candidateIds
          }
        }
      }
  ...
  private async finish(
    binding: ExecutionBinding,
    curated: MemoryCurationResult
  ) {
    if (curated.status === 'persisted') return
    const status =
      curated.status === 'deferred'
        ? binding.attempt >= this.maxAttempts
          ? 'quarantined'
          : 'retry'
        : curated.status === 'duplicate'
          ? 'completed'
          : 'skipped'
    await this.driver.executeQuery(
      `MATCH (b:MemoryBackgroundBatch {batchId:$batchId,claimId:$claimId,fence:$fence})
  ...
  private async release(binding: ExecutionBinding) {
    await this.driver.executeQuery(
       ...
       SET b.status=$status,b.reason='processing-failed',b.leaseUntil=null`,
      {
        ...
        claimId: binding.claimId,
        fence: binding.fence,
        status: binding.attempt >= this.maxAttempts ? 'quarantined' : 'retry'
      },
      { database: this.options.database }
    )
  }
  ...
  private async recordAttempt(
    ...
    const eventId = neo4jMemoryFingerprint([
      attemptId,
      finished ? 'model-finished' : 'model-intent'
    ])
    const event = createTextMemoryUsageEvent({
      ...
      operation: 'curation',
      workloadVersion: 'memory-background-v1',
      profileVersion: `memory-background-${binding.profile}-v1`,
      costClass: binding.profile === 'internal' ? 'experiment' : 'operational',
      ...
      providerUsage:
        parsed === null
          ...
          : {
              ...
              modelVersion: null,
              adapterVersion:
                this.options.extractor?.version ?? 'memory-background-v1',
              provenance:
                binding.scope.sourceKind === 'synthetic-transcript'
                  ? 'synthetic'
                  : 'provider-reported',
              inputTokens: parsed.inputTokens,
              outputTokens: parsed.outputTokens,
              totalTokens: parsed.totalTokens,
              inputTextTokens: parsed.inputTokens,
              outputTextTokens: parsed.outputTokens,
    ...
    const entry = MemoryUsageLedgerEntrySchema.parse(
      finished && this.options.priceUsage
        ? this.options.priceUsage(event, eventId)
        : createUnknownCostMemoryUsageLedgerEntry(event, eventId)
    )
    if (
      JSON.stringify(entry.usageEvent) !== JSON.stringify(event) ||
      entry.ledgerEntryId !== eventId
    )
      throw new Error('Background pricing changed its usage evidence')
    const ledgerScope = neo4jMemoryFingerprint([
      'memory-usage-scope-v1',
      binding.scope.tenantId,
      binding.scope.subjectId
    ])
    await this.transaction(async (transaction) => {
      await transaction.run(
        'MERGE (h:MemoryUsageLedgerHead {scopeKey:$scopeKey}) ON CREATE SET h.lockVersion=0 SET h.lockVersion=h.lockVersion+1',
        { scopeKey: ledgerScope }
      )
      const existing = await transaction.run(
        'MATCH (e:MemoryUsageLedgerEntry {identityKey:$identityKey}) RETURN e.serialized AS serialized',
      ...
      await transaction.run(
         ...
         SET a.finished=$finished,a.costKnown=$costKnown,a.usageEventId=$eventId`,
        {
          ...
          scopeKey: ledgerScope,
          identityKey: neo4jMemoryFingerprint([ledgerScope, 'entry', eventId]),
          eventIdentity: neo4jMemoryFingerprint([
            ledgerScope,
            'event',
            eventId
          ]),
          ...
          finished,
          eventId,
          costKnown: entry.cost.brlAmount !== null
        }
      )
    })
  }
  ...
  async metrics() {
    ...
    const attempts = await this.driver.executeQuery(
      `MATCH (a:MemoryBackgroundAttempt) RETURN count(a) AS attempts,
       ...
       count(CASE WHEN a.costKnown=false THEN 1 END) AS unknownCostAttempts`,
      {},
      { database: this.options.database }
    )
    return {
      ...batches.records[0]?.toObject(),
      ...attempts.records[0]?.toObject(),
...
export async function createNeo4jMemoryBackgroundRuntime(
  options: Neo4jMemoryBackgroundOptions
) {
  if (
    !options.uri ||
    !options.username ||
    ...
    throw new Error('Background Neo4j configuration is incomplete')
  const now = options.now ?? (() => new Date())
  const deadline = options.extractor?.deadlineMilliseconds ?? 30_000
  const leaseMs =
    options.claimMilliseconds ?? Math.max(60_000, deadline + 10_000)
  if (
    !Number.isSafeInteger(leaseMs) ||
    leaseMs <= deadline ||
    leaseMs > 600_000
  )
    throw new Error('Invalid background claim window')
  const maxAttempts = z
    .number()
    .int()
    .min(1)
  ...
  const store = new Neo4jBackgroundStore(
    driver,
    options,
    now,
    leaseMs,
  ...
  const engine =
    options.extractor === undefined
      ? null
      : new MemoryBackgroundEngine(
          store,
          options.extractor,
          now,
          new LangGraphMemoryBackgroundAdapter()
        )
  return {
    ingest: (
      scope: MemoryRequestScope,
      ...
      formationSignal: MemoryFormationSignal,
      profile: MemoryBackgroundProfile
    ) => store.ingest(scope, source, formationSignal, profile),
    ...
    process: async (
      ...
      if (engine === null)
        throw new Error(
          'Background Memory extractor is not configured for processing'
        )
