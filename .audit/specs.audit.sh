...
if ! grep -F '.agents/rules/*.rule.md' "$PROJECT_ROOT/AGENTS.md" >/dev/null 2>&1; then
...
if [ -f "$PROJECT_ROOT/.github/pr_template.md" ]; then
  spec_fail legacy-pr-template .github/pr_template.md \
...
if [ -f "$PR_TEMPLATE" ]; then
  for heading in \
    ...
    "## Merge gate"
  do
    grep -Fx "$heading" "$PR_TEMPLATE" >/dev/null 2>&1 ||
      spec_fail pr-template-section .github/pull_request_template.md \
...
if [ -n "$nested_dirs" ]; then
  while IFS= read -r nested_dir; do
    ...
    spec_fail flat-spec-catalog "${nested_dir#"$PROJECT_ROOT"/}" \
...
for path in "$PROJECT_ROOT"/.audit/*.sh; do
  ...
  case "$path" in
    ...
    *) spec_fail semantic-suffix "${path#"$PROJECT_ROOT"/}" \
      ...
      "rename the checker and update launcher/CI references atomically" ;;
  esac
done
...
if [ -n "$legacy_numbered" ]; then
  while IFS= read -r path; do
    ...
    spec_fail semantic-suffix "${path#"$PROJECT_ROOT"/}" \
      ...
      "rename the spec and update catalog/references atomically"
  done <<EOF
$legacy_numbered
EOF
fi
...
[ "$spec_count" -gt 0 ] ||
  spec_fail spec-presence .agents/specs \
    ...
    "create a NNN-lowercase-slug.spec.md document from the canonical template"
...
while IFS= read -r path; do
  ...
  then
    spec_fail catalog-filename "$file" \
      "every numbered spec must use priority 001-099 and the .spec.md semantic suffix" \
      "rename the file to 0NN-lowercase-slug.spec.md"
    continue
  fi
  ...
  if [ "$spec_mode" = retrospective ]; then
    ...
    [ "${integrity_length:-0}" -ge 80 ] ||
      spec_fail retrospective-integrity "$file" \
        ...
        "state reconstruction evidence and limits"
  fi
  ...
  grep -F "($base)" "$SPEC_ROOT/readme.md" >/dev/null 2>&1 ||
    spec_fail missing-index-entry "$file" \
      ...
      "add the exact semantic filename to .agents/specs/readme.md"
done <"$spec_files"
...
while IFS= read -r reference_file; do
  ...
  if grep -Eq '\.agents/specs/(ai|console|harness|history|landing|mobile|onboarding|product)/' "$reference_file"; then
    spec_fail stale-spec-reference "${reference_file#"$PROJECT_ROOT"/}" \
